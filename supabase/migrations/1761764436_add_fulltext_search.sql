...
BEGIN
  ...
  SELECT 
    ...
    ts_rank(j.content_search, plainto_tsquery('polish', search_query)) as rank
  FROM jokes j
  LEFT JOIN profiles p ON j.author_id = p.id
  LEFT JOIN categories c ON j.category_id = c.id
  WHERE 
    j.status = 'published'
    AND j.content_search @@ plainto_tsquery('polish', search_query)
    AND (p_category_id IS NULL OR j.category_id = p_category_id)
  ORDER BY
    CASE WHEN p_order_by = 'relevance' THEN ts_rank(j.content_search, plainto_tsquery('polish', search_query)) END DESC,
